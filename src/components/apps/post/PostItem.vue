<template>
  <q-item clickable :to="`/posts/${item.id}`" class="bg-white q-pt-md">
...
// const { id: postId, likeCount: initialCount } = toRefs();
...
const { state: postUser } = useAsyncState(() => getUserById(props.item.uid));
</script>
