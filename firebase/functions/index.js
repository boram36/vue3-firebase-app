...
const { logger } = require('firebase-functions/v2'); // 올바른 import 방식
...
exports.onCreateUser = functions
  ...
  .onCreate(user => {
    ...
    const defaultPhotoURL = `https://api.dicebear.com/9.x/glass/svg?seed=${user.uid}`;
...
exports.onUpdatePost = onDocumentUpdated(
  ...
  event => {
    ...
    const data = event.data.after.data(); // data.tags => vuejs, react, java, svelte

    const tagsToRemove = differenceTags(prevData.tags, data.tags);
    const tagsToAdd = differenceTags(data.tags, prevData.tags);
    logger.log('tagsRemove: ', tagsToRemove);
    logger.log('tagsAdd: ', tagsToAdd);
    if (tagsToRemove) {
